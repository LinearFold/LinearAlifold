...
inline int vrna_hamming_distance(const string s1,
                          const string s2) {
    int h = 0;
...
inline float **get_ribosum(const vector<string> &Alseq,
            int n_seq,
            int length) {
    int i, j, k;
    float ident = 0;
...
inline tuple<vector<vector<int>>, vector<vector<int>>, vector<vector<int>>, vector<vector<int>>> a2s_prepare_is(vector<std::string> &MSA, int n_seq, int MSA_seq_length, vector<vector<int>> &a2s, vector<vector<int>> &s5, vector<vector<int>> &s3, vector<vector<int>> &SS, vector<float> &smart_gap) {
...
// other 0
inline int get_AUCG_pair_type_CONTRAfold_indices(int l, int r) {
...
//  inline double make_pscores_ij(vector<int> &SS_fast_i, vector<int> &SS_fast_j, float **ribo, float beta, float delta) // malikap: fixed this function
// {
